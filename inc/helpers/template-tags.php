...
function mwebsite_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'mwebsite' ),
 ...
 * @return void
 */
function mwebsite_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'mwebsite' ),
 ...
 * @return bool|string
 */
function mwebsite_the_excerpt( $trim_character_count = 0) {
 ...
 */
function mwebsite_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="mwebsite-read-more text-white" href="%1$s"><button class="mt-3 btn btn-info">%2$s</button></a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'mwebsite' )
		);
	}
...
function mwebsite_pagination() {
	...
	printf( '<nav class="mwebsite-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
